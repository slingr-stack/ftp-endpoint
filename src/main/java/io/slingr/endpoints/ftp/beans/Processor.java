...
import io.slingr.endpoints.ftp.components.CustomSftpComponent;
...
public class Processor extends RouteBuilder {
    ...
    public Processor(AppLogs appLogs, Events events, Files files, String name, boolean localDeployment,
        ...
        logger.info(String.format("Configured FTP endpoint [%s://%s@%s:%s]", protocol, username, host, port));
    ...
    public void configure() throws Exception {
        ...
        logger.info(String.format("Starting FTP endpoint with parameters: %s", parametersToPrint.toString()));
     ...
     * Also if the exception is of type {@link EndpointException} it will return details of the
    ...
    public ValueBuilder exceptionJsonDetails() {
        Expression expression = new ExpressionAdapter() {
            public Object evaluate(Exchange exchange) {
                ...
                String message = "An exception happened in the endpoint";
                ...
                if (exception != null) {
                    ...
                    if (exception instanceof EndpointException) {
                        code = ((EndpointException) exception).getCode();
                        if(!((EndpointException) exception).getAdditionalInfo().isEmpty()) {
                            additionalInformation = ((EndpointException) exception).getAdditionalInfo();
                        }
                    }
                }

                return EndpointException.permanent(code, message, additionalInformation).toJson(true);
            }
