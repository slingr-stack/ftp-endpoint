package io.slingr.endpoints.ftp.beans;

import io.slingr.endpoints.exceptions.EndpointException;
import io.slingr.endpoints.exceptions.ErrorCode;
import io.slingr.endpoints.ftp.utils.EmailHelper;
...
public class FilesService {
    ...
    @Handler @SuppressWarnings("unused") // used on Endpoint routes
    /**
    ...
    public Json newFile(@Body InputStream is, @Headers Map headers){
        ...
        try {
            ...
            throw EndpointException.permanent(ErrorCode.CLIENT, message, ex);
        }
    }
    ...
    public void downloadFile(@Body Json body, @Headers Map<String, String> headers) {
        ...
        if(StringUtils.isBlank(fileId)){
            throw EndpointException.permanent(ErrorCode.ARGUMENT, "Empty file id");
        } else {
            fileId = fileId.trim();
        }
        ...
        if(dwnFile == null || dwnFile.getFile() == null){
            final EndpointException re = EndpointException.permanent(ErrorCode.CLIENT, String.format("It is not possible to download the file [%s] from application to endpoint. The file will not be uploaded to ftp.", fileName));
            logger.warn(re.getMessage());
            throw re;
        }
        final File tmp = FilesUtils.copyInputStreamToTemporaryFile(fileName, dwnFile.getFile());
        if(tmp == null || !tmp.exists()){
            final EndpointException re = EndpointException.permanent(ErrorCode.CLIENT, String.format("It is not possible to copy the file [%s] from application to endpoint. The file will not be uploaded to ftp.", fileName));
            logger.warn(re.getMessage());
            throw re;
        }
        final long fileLength = tmp.length();
        if(fileLength < 1){
            final EndpointException re = EndpointException.permanent(ErrorCode.CLIENT, String.format("The copy of the file [%s] on endpoint is empty. The file will not be uploaded to ftp.", fileName));
            logger.warn(re.getMessage());
            throw re;
        }
    ...
    public void uploadFile(Exchange exchange, @Header(HEADER_LOCAL_FILE_PATH) String localFilePath) {
        if(StringUtils.isBlank(localFilePath)){
            final EndpointException re = EndpointException.permanent(ErrorCode.CLIENT, String.format("The copy of the file [%s] on endpoint is invalid. The file will not be uploaded to ftp.", localFilePath));
            logger.warn(re.getMessage());
            throw re;
        }
